...
The Pine script posts `entry` (the FVG midpoint), `stop` (beyond the swept liquidity plus 0.1 ATR), `target_liquidity` (the next untouched opposing pool), the FVG, order block, range, sweep and MSS levels, HTF bias and killzone. It also carries its **rule score** output: `score`, a 0-100 agreement score from weighted inputs (HTF bias, sweep, shift with displacement, FVG, discount/premium, killzone, order block; weights are inputs and a minimum score gates the alert), plus `hit_rate` and `samples`, the chart's own simulated memory of past setups. On the chart the indicator separates a **SETUP** (the webhook event, drawn as a plan: FVG midpoint to watch, stop, TP1 at the liquidity pool, TP2-TP4 as R references) from a later **BUY / SELL confirmed** marker, which requires a candle to touch the midpoint and close back beyond the gap while the direction, session, location and risk filters still pass. Only confirmed entries feed `hit_rate`; expired, cancelled, missed and ambiguous plans are counted separately and excluded. Confirmed entries and outcomes are exposed as named `alertcondition`s for phone notifications; the webhook's *Any alert() function call* still receives only the setup JSON. `tradingview/SMC_GUIDE.md` explains every label in plain language. Pine cannot run a language model; the Ollama analysis stays server-side and returns through Telegram, using these fields as extra context. The analyzer re-validates every relation server-side (`entry_outside_fvg`, `stop_not_beyond_sweep`, `structure_not_shifted`, `entry_not_in_discount`/`entry_not_in_premium`, `risk_outside_atr_band` for 0.3-3 ATR, `target_too_close`, `htf_bias_mismatch`, `outside_killzone`) before the model sees it. Messages show TP1 at the liquidity target with its R multiple, then the configured R multiples beyond it. `python3 scripts/demo.py BTCUSD smc` sends a synthetic pair through the SMC path. Neither model has established performance; both remain manual-review tools.
